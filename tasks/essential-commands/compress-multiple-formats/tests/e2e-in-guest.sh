...
state=/var/lib/lfcs-simulator/compress-multiple-formats-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-compress-multiple-formats" "$state"
    export LFCS_TASK_ID=essential-commands/compress-multiple-formats LFCS_SEED=e2e-compress-multiple-formats LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
...
assert r["score"] == r["max_score"] == 6, r
PY
