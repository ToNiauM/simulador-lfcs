...
state=/var/lib/lfcs-simulator/redirect-command-output-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-redirect-command-output" "$state"
    export LFCS_TASK_ID=essential-commands/redirect-command-output LFCS_SEED=e2e-redirect-command-output LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
...
assert r["score"] == r["max_score"] == 5, r
PY
