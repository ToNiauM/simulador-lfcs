...
state=/var/lib/lfcs-simulator/git-init-first-commit-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-git-init-first-commit" "$state"
    export LFCS_TASK_ID=essential-commands/git-init-first-commit LFCS_SEED=e2e-git-init-first-commit LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
...
assert r["score"] == r["max_score"] == 8, r
PY
