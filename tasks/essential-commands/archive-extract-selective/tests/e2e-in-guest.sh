...
state=/var/lib/lfcs-simulator/archive-extract-selective-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-archive-extract-selective" "$state"
    export LFCS_TASK_ID=essential-commands/archive-extract-selective LFCS_SEED=e2e-archive-extract-selective LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
