...
state=/var/lib/lfcs-simulator/find-large-files-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-find-large-files" "$state"
    export LFCS_TASK_ID=essential-commands/find-large-files LFCS_SEED=e2e-find-large-files LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
