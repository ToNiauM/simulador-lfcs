...
state=/var/lib/lfcs-simulator/cut-fields-report-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-cut-fields-report" "$state"
    export LFCS_TASK_ID=essential-commands/cut-fields-report LFCS_SEED=e2e-cut-fields-report LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
