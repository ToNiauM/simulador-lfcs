...
state=/var/lib/lfcs-simulator/gpt-partition-layout-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-gpt-partition-layout" "$state"
    export LFCS_TASK_ID=storage/gpt-partition-layout LFCS_SEED=e2e-gpt-partition-layout LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
