...
state=/var/lib/lfcs-simulator/mdadm-raid1-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-mdadm-raid1" "$state"
    export LFCS_TASK_ID=storage/mdadm-raid1 LFCS_SEED=e2e-mdadm-raid1 LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
