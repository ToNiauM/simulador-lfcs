...
state=/var/lib/lfcs-simulator/vg-add-second-pv-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-vg-add-second-pv" "$state"
    export LFCS_TASK_ID=storage/vg-add-second-pv LFCS_SEED=e2e-vg-add-second-pv LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
