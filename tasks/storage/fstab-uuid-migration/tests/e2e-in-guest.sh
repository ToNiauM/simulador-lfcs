...
state=/var/lib/lfcs-simulator/fstab-uuid-migration-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-fstab-uuid-migration" "$state"
    export LFCS_TASK_ID=storage/fstab-uuid-migration LFCS_SEED=e2e-fstab-uuid-migration LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
