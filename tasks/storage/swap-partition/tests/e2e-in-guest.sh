...
state=/var/lib/lfcs-simulator/swap-partition-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-swap-partition" "$state"
    export LFCS_TASK_ID=storage/swap-partition LFCS_SEED=e2e-swap-partition LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
