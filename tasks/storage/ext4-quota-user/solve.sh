...
hard="$(task_param block_hard_kib)"
mount_point="$(task_param mount_point)"

parted -s "$disk" mklabel gpt mkpart quota 1MiB "$((size + 1))MiB"
udevadm settle
part="${disk}1"
mkfs -t "$fs" -F "$part"
mkdir -p "$mount_point"
...
printf 'UUID=%s %s %s defaults,usrquota 0 2\n' "$uuid" "$mount_point" "$fs" >> /etc/fstab
mount "$mount_point"
quotacheck -cum "$mount_point"
quotaon "$mount_point"
setquota -u "$user" 0 "$hard" 0 0 "$mount_point"
