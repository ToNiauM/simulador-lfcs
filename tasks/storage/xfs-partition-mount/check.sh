...
[[ -n "${LFCS_PARAMS_FILE:-}" && -r "$LFCS_PARAMS_FILE" ]] || { echo '{"schema_version":1,"task_id":"storage/xfs-partition-mount","result":"error","score":0,"max_score":10,"criteria":[{"id":"parameters","result":"error","points":0,"max_points":10,"message":"missing parameters","evidence":"LFCS_PARAMS_FILE unavailable"}]}' ; exit 0; }
...
c1 = criterion("gpt_label", table.get("label", "") == "gpt", 2, "disk uses a GPT partition table", table.get("label") or "no partition table")
c2 = criterion("part_size", len(parts) >= 1 and abs(sizes[0] - expected_size) <= 4, 2, f"partition of {expected_size} MiB exists on the task disk", f"sizes={sizes}" if sizes else "no partitions")
...
c3 = criterion("mounted_filesystem", source_on_disk and active_fs == fs, 3, f"{fs} filesystem from the task disk is mounted at the requested mount point", f"{active_source or 'not mounted'} ({active_fs or '-'})")
persistent = False
...
if device and os.path.exists("/etc/fstab"):
    ...
    for line in open("/etc/fstab"):
        ...
        if source_matches:
            ...
            evidence = line.strip()
            break
c4 = criterion("persistent_fstab", persistent, 3, "fstab restores the mount", evidence)
