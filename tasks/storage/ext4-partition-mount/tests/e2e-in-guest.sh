...
state=/var/lib/lfcs-simulator/ext4-partition-mount-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-ext4-partition-mount" "$state"
    export LFCS_TASK_ID=storage/ext4-partition-mount LFCS_SEED=e2e-ext4-partition-mount LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
