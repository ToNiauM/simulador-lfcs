...
state=/var/lib/lfcs-simulator/ssh-harden-config-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-ssh-harden-config" "$state"
    export LFCS_TASK_ID=networking/ssh-harden-config LFCS_SEED=e2e-ssh-harden-config LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
