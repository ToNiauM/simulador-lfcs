...
state=/var/lib/lfcs-simulator/system-proxy-env-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-system-proxy-env" "$state"
    export LFCS_TASK_ID=networking/system-proxy-env LFCS_SEED=e2e-system-proxy-env LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
