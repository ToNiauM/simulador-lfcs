...
state=/var/lib/lfcs-simulator/hostname-and-hosts-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-hostname-and-hosts" "$state"
    export LFCS_TASK_ID=networking/hostname-and-hosts LFCS_SEED=e2e-hostname-and-hosts LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
