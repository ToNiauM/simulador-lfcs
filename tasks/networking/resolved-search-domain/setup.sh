...
# Idempotency: remove any previous configuration of ours carrying the seeded
# domain, whichever mechanism produced it (netplan, networkd or NetworkManager).
while IFS= read -r conf; do
  rm -f "$conf"
done < <(grep -l -- "$domain1" /etc/netplan/*.yaml /etc/netplan/*.yml \
