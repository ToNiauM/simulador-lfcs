...
state=/var/lib/lfcs-simulator/vlan-interface-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-vlan-interface" "$state"
    export LFCS_TASK_ID=networking/vlan-interface LFCS_SEED=e2e-vlan-interface LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
