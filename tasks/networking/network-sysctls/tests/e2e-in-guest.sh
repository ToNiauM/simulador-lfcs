...
state=/var/lib/lfcs-simulator/network-sysctls-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-network-sysctls" "$state"
    export LFCS_TASK_ID=networking/network-sysctls LFCS_SEED=e2e-network-sysctls LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
