...
state=/var/lib/lfcs-simulator/second-nic-static-dns-full-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-second-nic-static-dns-full" "$state"
    export LFCS_TASK_ID=networking/second-nic-static-dns-full LFCS_SEED=e2e-second-nic-static-dns-full LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
