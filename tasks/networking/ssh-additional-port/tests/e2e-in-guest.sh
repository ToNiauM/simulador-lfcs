...
state=/var/lib/lfcs-simulator/ssh-additional-port-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-ssh-additional-port" "$state"
    export LFCS_TASK_ID=networking/ssh-additional-port LFCS_SEED=e2e-ssh-additional-port LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
