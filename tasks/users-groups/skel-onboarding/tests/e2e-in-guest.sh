...
state=/var/lib/lfcs-simulator/skel-onboarding-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-skel-onboarding" "$state"
    export LFCS_TASK_ID=users-groups/skel-onboarding LFCS_SEED=e2e-skel-onboarding LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
