...
state=/var/lib/lfcs-simulator/cron-job-user-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-cron-job-user" "$state"
    export LFCS_TASK_ID=operations-deployment/cron-job-user LFCS_SEED=e2e-cron-job-user LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
