...
state=/var/lib/lfcs-simulator/timezone-locale-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-timezone-locale" "$state"
    export LFCS_TASK_ID=operations-deployment/timezone-locale LFCS_SEED=e2e-timezone-locale LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
