...
state=/var/lib/lfcs-simulator/default-boot-target-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-default-boot-target" "$state"
    export LFCS_TASK_ID=operations-deployment/default-boot-target LFCS_SEED=e2e-default-boot-target LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
