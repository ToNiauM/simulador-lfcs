...
state=/var/lib/lfcs-simulator/service-resource-limits-e2e.json
[[ "${LFCS_GUEST_LAB:-}" == "1" && "$(id -u)" == 0 ]] || { echo "run only as root in disposable guest" >&2; exit 64; }

case "$phase" in
  prepare)
    mkdir -p "$(dirname "$state")"
    "$pack/../../../bin/render-task" "$pack" "e2e-service-resource-limits" "$state"
    export LFCS_TASK_ID=operations-deployment/service-resource-limits LFCS_SEED=e2e-service-resource-limits LFCS_PARAMS_FILE="$state"
    "$pack/setup.sh"
